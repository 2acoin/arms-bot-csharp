...
//            Response.AddInlineField("Data as of", string.Format("{0}", cachedDate));
            ...
            Response.AddInlineField(TrtlBotSharp.coinSymbol + "-USD", string.Format("${0:N5} USD", (decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[7]) * (decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[10])));
            Response.AddInlineField("Volume BTC/USD", string.Format("{0:F4}/{1:C}", ((decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[8]) * (decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[9])), ((decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[8]) * (decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[9])) * (decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[10])));
            Response.AddInlineField("BTC-USD", string.Format("{0:C} USD", (decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[10])));
            DateTime cachedDate = DateTime.Parse(TrtlBotSharp.marketCacheArray[0]);
            Response.AddInlineField("Data as of", string.Format("{0}", cachedDate));
...
//            decimal CoinPrice = (decimal)Convert.ToDecimal(TrtlBotSharp.marketCacheArray[3]);
            ...
            string Response = string.Format("{0}'s market cap is **{1:c}** USD", TrtlBotSharp.coinName,
                ((decimal)CoinPrice * (decimal)decimal.Parse(TrtlBotSharp.marketCacheArray[10])) * TrtlBotSharp.GetSupply());
